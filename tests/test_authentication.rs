...
async fn test_authentication_success() {
    ...
    assert!(env.is_ok(), "Should authenticate successfully with valid credentials");
    
    let env = env.unwrap();
    assert!(env.client.is_authenticated(), "Client should be authenticated");
    assert!(env.client.get_token().is_some(), "Should have authentication token");
}
...
async fn test_authentication_failure() {
    ...
    let result = client.authenticate("invalid_user".to_string(), "wrong_password".to_string()).await;
    ...
    assert!(client.get_token().is_none(), "Should not have authentication token");
}
...
async fn test_token_persistence() {
    ...
    assert!(env2.client.is_authenticated(), "Second client should be authenticated");
